...
/** Keywords that trigger Task-specific hints in the system prompt. */
const TASK_KEYWORDS = /task\.v2|タスク.*作成|作成.*タスク|task.*create|create.*task/i;

/** Keywords that trigger Calendar-specific hints in the system prompt. */
const CALENDAR_KEYWORDS = /calendar\.v4|カレンダー|予定|スケジュール|今日の.*予定|明日の.*予定|calendar|schedule|予定|予定を確認/i;
...
/**
 * Task-specific guidance injected only when the user message is task-related.
 */
export const TASK_HINTS = `
task.v2.task.create を使う際の重要ルール:
- タイトルは "title" ではなく "summary" フィールドに渡す（summaryは必須）
- due（締切）を設定する場合: due.timestamp はUTCエポックからの**ミリ秒**を**文字列**で渡す
  - 例: 2026/3/26 → new Date('2026-03-26T00:00:00+09:00').getTime() = "1774396800000"
  - 終日タスクの場合は due.is_all_day: true も設定する
  - 秒ではなくミリ秒であることに注意（13桁の数字）
...
export const TOOL_CALL_FORMAT_HINT = `重要: tool call の arguments は必ず厳密なJSON objectを出力してください。XML風タグ（<arg_key>, <arg_value>）や key=value 連結形式は使用禁止です。
...
/**
 * Build the static (cacheable) part of the system prompt.
 * This part should remain consistent across requests to maximize cache hits.
 */
export function buildStaticSystemPrompt(
  functions: FunctionDefinition[],
  hasBitableKeywords: boolean,
  hasTaskKeywords = false,
  hasCalendarKeywords = false
): string {
  const toolDocs = functions.map(f => `- ${f.function.name}: ${f.function.description}`).join('\n');
  const domainHints = [
    hasBitableKeywords ? BITABLE_HINTS : '',
    hasTaskKeywords ? TASK_HINTS : '',
    hasCalendarKeywords ? CALENDAR_HINTS : '',
  ].filter(Boolean).join('\n');
  const skillGuide = LARK_MCP_SKILL_GUIDE ? `## Lark MCP 使用ガイド\n${LARK_MCP_SKILL_GUIDE}` : '';
...
/**
 * Build the dynamic (non-cacheable) part of the system prompt.
 * Contains context-specific information like datetime and planner hints.
 */
export function buildDynamicSystemPrompt(intentPlan: IntentPlan, userText = ''): string {
  const parts: string[] = [];
  ...
  if (plannerHints) {
    parts.push(plannerHints);
  }

  if (DATETIME_KEYWORDS.test(userText)) {
    const dateTimeStr = new Date().toLocaleString('ja-JP', {
      timeZone: 'Asia/Tokyo',
      year: 'numeric', month: '2-digit', day: '2-digit',
      hour: '2-digit', minute: '2-digit', weekday: 'short',
    });
    parts.push(`現在日時: ${dateTimeStr}（Asia/Tokyo）`);
  }
 ...
 * Splits into static (cacheable) and dynamic parts to improve cache efficiency.
 */
export function buildSystemPrompt(
  ...
  const hasBitableKeywords = BITABLE_KEYWORDS.test(userText);
  const hasTaskKeywords = TASK_KEYWORDS.test(userText);
  const hasCalendarKeywords = CALENDAR_KEYWORDS.test(userText);
  const staticPrompt = buildStaticSystemPrompt(functions, hasBitableKeywords, hasTaskKeywords, hasCalendarKeywords);
  const dynamicPrompt = buildDynamicSystemPrompt(intentPlan, userText);
